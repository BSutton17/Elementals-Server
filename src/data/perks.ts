...
/** How many perks each player picks before they can ready up. */
...
export function hasFullPerkSelection(
  ...
  return (perks?.length ?? 0) === PERKS_PER_PLAYER;
}
 ...
 * `PERKS_PER_PLAYER` known, distinct perk ids. Returns null when the payload is
 ...
 */
export function normalizePerks(raw: unknown): PerkId[] | null {
  if (!Array.isArray(raw)) return null;
  if (raw.length > PERKS_PER_PLAYER) return null;
  const perks: PerkId[] = [];
